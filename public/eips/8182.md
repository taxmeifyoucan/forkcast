...
6. A **proof verification precompile** for gas-feasible proof verification.
...
* **Output note data**: Opaque per-output bytes emitted by the contract for wallet/app-layer note delivery. The base protocol does not validate or interpret these bytes. Section 14 defines the scheme-`1` interpretation. Delivery may also be coordinated out of band.
...
**Outer circuit** (hard-fork-managed). There is exactly one outer circuit; it can only change via hard fork. It enforces all protocol invariants for `transact`: value conservation, nullifier derivation, Merkle membership, deterministic note-secret derivation for ordinary outputs, user-registry checks, auth-policy checks, and token consistency. It also recursively verifies an inner proof as part of its own verification. The outer circuit is the security boundary — a bug here can compromise the entire pool.

**Inner circuit** (permissionless). Anyone can write and deploy an inner circuit. It handles authentication — verifying the user's credential — and intent parsing — computing the transaction intent digest over transaction fields and any signer-selected execution constraints. It outputs two public values: `[authDataCommitment, transactionIntentDigest]`. The outer circuit uses private witnesses for `authorizingAddress` and `policyVersion`, authenticates them through `transactionIntentDigest`, checks `authDataCommitment` against the auth-policy leaf, and checks `transactionIntentDigest` against its own independent recomputation from execution data plus finalized-output bindings. Section 9 specifies the full per-mode constraints.

**How they compose.** A prover supplies the inner proof and the inner verification key (`innerVkey`) as private witnesses to the outer circuit. The outer circuit recursively verifies the inner proof, computes `innerVkHash` from `innerVkey`, and uses it to look up the auth policy registry leaf. Because `innerVkey` is a private witness, on-chain observers cannot determine which inner circuit (and therefore which auth method) was used for a given private-note spend. Section 9.1 specifies the full normative interface.
...
A candidate root `r` is accepted iff there exists a stored pair `(storedRoot, storedBlockNumber)` such that `storedRoot == r` and `block.number - storedBlockNumber <= W`. The current root is always accepted. For the user registry and auth policy registry, `r = 0` is never accepted, regardless of history contents.
...
The auth policy registry binds `(address, innerVkHash)` pairs to credentials. State layout is specified in Section 5.2. Registration is via `registerAuthPolicy`, called directly by the account holder. See Section 5.3.
...
2. compute `innerVkHash` from `innerVkey`,
...
9. enforce value conservation and token consistency.
...
`innerVkHash` is not an inner-circuit output; the outer circuit computes it from `innerVkey`, the `UltraHonkVerificationKey` passed to the recursive verifier. Let `vk[0] .. vk[114]` denote the 115 BN254 field elements of `innerVkey` in recursive-verifier order. Then:
...
For each output slot `i`, where `i ∈ {0, 1, 2}` indexes public outputs `noteBodyCommitment0`, `noteBodyCommitment1`, and `noteBodyCommitment2`, the circuit witnesses slot-local fields `ownerNullifierKeyHash_i`, `noteSecret_i`, `amount_i`, and `tokenAddress_i`, together with a per-slot `isDummy_i` flag constrained to `0` or `1`. In this subsection, subscripted fields such as `amount_i` refer to output-slot fields; un-subscripted `amount` refers to the transaction-intent amount.
...
The outer and inner circuits do not validate encryption scheme semantics or delivery format. Section 14 defines the registry lookup and the interpretation for scheme ID `1`.
...
These fields are authenticated directly as inputs to `transactionIntentDigest` in Section 9.10.
...
A valid `transact` therefore has one of two configurations:

* **Unconstrained** — no output-binding locks are set.
* **Finalized-authorization** — one or more output-binding locks are set.
...
`executionConstraintsFlags`, `lockedOutputBinding0`, `lockedOutputBinding1`, `lockedOutputBinding2`, `nonce`, and `transactionIntentDigest` are not public inputs. They are private signed/authenticated values checked inside the recursive proof relation.
...
| Transaction intent digest | `TRANSACTION_INTENT_DIGEST_DOMAIN, policyVersion, authorizingAddress, operationKind, tokenAddress, recipientAddress, amount, feeRecipientAddress, feeAmount, executionConstraintsFlags, lockedOutputBinding0, lockedOutputBinding1, lockedOutputBinding2, nonce, validUntilSeconds, executionChainId` | 16 |
...
### 13. Example Single-Circuit ECDSA Companion Standard (Non-Normative)
...
4. Enforces `nonce < p`.
5. Fixes the execution constraints to the unconstrained values `executionConstraintsFlags = 0`, `lockedOutputBinding0 = 0`, `lockedOutputBinding1 = 0`, and `lockedOutputBinding2 = 0`, then computes `transactionIntentDigest` per Section 9.10 using `executionChainId`, the derived `authorizingAddress`, the signed `policyVersion`, and those fixed values.
...
### 14. Output Note Data and Delivery Keys
...
#### 14.1 Scheme IDs and Support Requirements
...
#### 14.2 Scheme 1A: `transact` Output Payloads
...
#### 14.3 Scheme 1B: Deposit Payloads
...
Deposits and withdrawals are public by design. Deposits reveal depositor, token, and amount. Private transfers keep token and amount private. Output note data may leak metadata depending on the delivery scheme and wallet payload conventions in use.
