...
6. derive `intentReplayId`,
7. validate input note ownership and nullifiers,
8. validate output note commitments and output bindings,
...
`outputNoteData` for this subtype MUST be exactly 1296 bytes and encoded as `enc || ciphertext || tag`, where:

* `enc` is the first 1120 bytes and is the raw X-Wing ciphertext,
...
`outputNoteData` for this subtype MUST be exactly 1200 bytes and encoded as `enc || ciphertext || tag`, where:
