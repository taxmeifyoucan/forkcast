...
The two state-gas products change whenever `CPSB` changes. Execution-gas and state-gas semantics are defined by [EIP-8037](./eip-8037.md).
...
Blocks of pure ETH transfers should be tested across all EL clients (e.g. on Perfnet) to confirm the pricing matches measured run times at the chosen throughput anchor.
