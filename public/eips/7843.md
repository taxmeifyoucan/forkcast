...
New objects `ExecutionPayloadV4` and `PayloadAttributesV4` are introduced, adding a `slotNumber` field of type `uint64`. The methods `engine_newPayloadV5`, `engine_getPayloadV6`, and `engine_forkchoiceUpdatedV4` are added, to use the new objects.
