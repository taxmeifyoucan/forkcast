...
- On an address collision ([EIP-684](./eip-684.md)) the creation fails as an exceptional halt of the create frame, consuming the forwarded gas. A colliding destination has a nonzero nonce or non-empty code, so it is existent and no account-creation state-gas was charged. A nonzero balance or non-empty storage alone does not cause a collision.
...
The same rules apply when the top-level call frame reverts or halts, treating the frame as a child of the transaction boundary. In particular, for address collisions in contract-creation transactions, `gas_left` is consumed in full. The colliding destination has a nonzero nonce or non-empty code ([EIP-684](./eip-684.md)), so no account-creation state-gas was charged. The `state_gas_reservoir` is returned to the sender by the normal end-of-transaction settlement.
...
| ----------------------------------------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------- | ---------- |
| Deploy 24 kB contract ([EIP-170](./eip-170.md) limit)        | Runtime code + account trie node               | 32,000 CREATE + 200 × 24,576 code deposit = 4,947,200 gas                | 24,696 B      | ~200 gas   |
...
The conditional account-creation charge is compatible with this by construction. The existence read that decides the charge is the *same single access* that performs the [EIP-684](./eip-684.md) collision check, and it happens after the pre-access failure checks — exactly the access [EIP-7928](./eip-7928.md) already sanctions and records. Creations that fail before access charge nothing and appear nowhere in the block access list; creations that reach the access point are recorded and pay exactly for the state they are about to create. The recorded access set is identical to that of an unconditional-charge design: the charge decision introduces no additional read.
...
A remaining gap is value-bearing *normal transactions* to an empty account, which trigger account creation but are not currently charged for it. This is addressed by [EIP-2780](./eip-2780.md), which adds the account-creation cost to the intrinsic gas of such transactions (see [Pricing mismatch with respect to ETH transfers](#pricing-mismatch-with-respect-to-eth-transfers)).
...
### Pricing mismatch with respect to ETH transfers
...
[EIP-2780](./eip-2780.md) solves this mismatch by adding a new component to the intrinsic gas cost of transactions. If a non-create transaction has value > 0 and targets a non-existent account, the `GAS_NEW_ACCOUNT` is added to intrinsic cost.
