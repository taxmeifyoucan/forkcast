...
**Implicit EOA authorization**: An unregistered actor (`actor_config` slot is empty) is implicitly authorized if `actorId == bytes32(bytes20(account))` **and** the account's `DEFAULT_EOA_REVOKED` flag is not set. The empty slot's scope byte and `policyType` are both `0x00`, and `expiry` is `0` (never expires), granting unrestricted, non-expiring permissions and no protocol-enforced policy by default. This allows every existing EOA to send AA transactions immediately without prior registration. When the implicit rule applies, the protocol authenticates using native ecrecover rather than calling an external authenticator contract.

The implicit authorization is disabled by setting the `DEFAULT_EOA_REVOKED` flag bit in the packed account-state slot (alongside the change sequences and lock state — see [Account Lock](#account-lock)); the self-actor's `actor_config` slot stays empty. The EOA actor can also be explicitly registered with `K1_AUTHENTICATOR` (`address(1)`) to set a custom scope or `policyType` while retaining native ecrecover authentication; doing so also sets the flag, so an explicit (possibly scoped) self-config always supersedes the implicit full-owner path. This holds the invariant **an explicit self-actor entry implies the flag is set**, which keeps the full-owner short-circuit from ever shadowing a downgraded key. The flag is monotonic — once set it is not cleared — so a managed self key is always operated through its explicit `actor_config` entry rather than the zero-storage implicit path. `createAccount` and `importAccount` set this flag by default, so a newly created or imported account does not leave a native secp256k1 owner live unless one is explicitly registered, a quantum safe default.
...
Account lock state is stored in a single packed 32-byte account-state slot that also holds the change sequences and an account-flags byte:
...
| `flags` | Account flags byte; bit 0 (`DEFAULT_EOA_REVOKED`) disables the implicit default-EOA path |
...
3. **Authorize**: SLOAD `actor_config(sender, actorId)`. **Implicit EOA rule**: if the slot is empty, `actorId == bytes32(bytes20(sender))`, the account's `DEFAULT_EOA_REVOKED` flag is **not** set, and authentication in step 2 used the native secp256k1 path (the EOA path or `K1_AUTHENTICATOR`), treat as implicitly authorized with `scope = 0x00` and `policyType = 0x00`. Otherwise, require that the stored authenticator address matches the effective authenticator. If the stored `expiry` is non-zero, also require `block.timestamp <= expiry`; an expired actor is rejected.
