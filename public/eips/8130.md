...
| `ECRECOVER_VERIFIER` | `address(1)` | Native secp256k1 (ECDSA) verifier sentinel |
...
| `NONCE_KEY_MAX` | `2^192 - 1` | Nonce-free mode (expiry-only replay protection) |
...
**Implicit EOA authorization**: An unregistered owner (`owner_config` slot is empty) is implicitly authorized if `ownerId == bytes32(bytes20(account))`. The empty slot's scope byte is `0x00` (unrestricted), granting full permissions by default. This allows every existing EOA to send AA transactions immediately without prior registration. When the implicit rule applies, the protocol verifies using native ecrecover rather than calling an external verifier contract. The implicit authorization is revoked by writing a revocation marker to the slot (`verifier = address(0)` with non-zero scope), making it non-empty. The EOA owner can also be explicitly registered with `ECRECOVER_VERIFIER` to set a custom scope while retaining native ecrecover verification.
...
The transaction carries a single packed `nonce` field (`uint256`) that encodes:

- `nonce_key` (`uint192`) in the high 192 bits
- `nonce_sequence` (`uint64`) in the low 64 bits

```
nonce_key = uint192(nonce >> 64)
nonce_sequence = uint64(nonce)
nonce = (uint256(nonce_key) << 64) | uint256(nonce_sequence)
```
...
Verifiers are executed via STATICCALL. Verifier addresses MUST NOT be [EIP-7702](./eip-7702.md) delegations — reject if the code at the verifier address starts with the delegation designator (`0xef0100`). Execution is metered (see [Mempool Acceptance](#mempool-acceptance) for rules).
...
`ECRECOVER_VERIFIER` (`address(1)`) is a protocol-reserved sentinel for native secp256k1 verification. When the protocol encounters this address as a verifier in auth data, it performs ecrecover directly rather than making a STATICCALL. The `data` portion is interpreted as raw ECDSA `(r || s || v)`, and the returned `ownerId` is `bytes32(bytes20(recovered_address))`.
...
AA_TX_TYPE || rlp([
  ...
  nonce,              // packed uint256: nonce_key (high 192 bits) || nonce_sequence (low 64 bits)
...
| `nonce` | Packed `uint256` 2D nonce: `nonce_key = uint192(nonce >> 64)`, `nonce_sequence = uint64(nonce)`. Must match current sequence for `(from, nonce_key)`. Incremented after inclusion regardless of execution outcome. For nonce-free mode: `nonce_key == NONCE_KEY_MAX` and `nonce_sequence == 0`. |
...
**`sender_auth_cost`**: For EOA signatures (`from` empty) or native ecrecover signatures (`ECRECOVER_VERIFIER`): 6,000 gas (ecrecover + 1 SLOAD + overhead). For other configured owner signatures (`from` set): 1 SLOAD (`owner_config`) + cold code access + actual gas consumed by verifier execution.
...
| `nonce_key_cost` | 0 for `NONCE_KEY_MAX` (no nonce state accessed). Otherwise: 22,100 gas for first use of a `nonce_key` (cold SLOAD + SSTORE set), 5,000 gas for existing keys (cold SLOAD + warm SSTORE reset) |
...
1. **Resolve sender**: If `from` empty, ecrecover derives the sender address (EOA path). The effective verifier is `ECRECOVER_VERIFIER` and `ownerId = bytes32(bytes20(sender))`. If `from` set, read the first 20 bytes of `sender_auth` as the verifier address.
...
3. **Verify**: If the verifier is `ECRECOVER_VERIFIER`, the protocol natively ecrecovers from `data` (as `r || s || v`), returning `ownerId = bytes32(bytes20(recovered_address))`. For the EOA path, ecrecover was already performed in step 1. For all other verifiers, call `verifier.verify(hash, data)` via STATICCALL. Returns `ownerId` (or `bytes32(0)` for invalid).
4. **Authorize**: SLOAD `owner_config(from, ownerId)`. **Implicit EOA rule**: if the slot is empty and `ownerId == bytes32(bytes20(from))`, treat as implicitly authorized with scope `0x00`. Otherwise, require that the stored verifier address matches the effective verifier.
5. **Check scope**: Read the scope byte from `owner_config` (or `0x00` for the implicit case). Determine the context bit: `0x02` (SENDER) for `sender_auth`, `0x04` (PAYER) for `payer_auth`, `0x01` (SIGNATURE) for `verifySignature()`, `0x08` (CONFIG) for config change `authorizer_auth`. Require `scope == 0x00 || (scope & context_bit) != 0`.
...
The `code` field contains runtime bytecode placed directly at the account address. For [EIP-7702](./eip-7702.md)-style delegation, use a delegation entry (type `0x02`) in `account_changes` after account creation.
...
| `0x02` | `revokeOwner` | Revoke an existing owner — deletes the slot for non-EOA owners; for the implicit EOA owner (`ownerId == bytes32(bytes20(account))`), overwrites with `verifier = address(0)` and non-zero scope to prevent implicit re-authorization | `ownerId` |
...
7. Decode packed nonce: `nonce_key = uint192(nonce >> 64)`, `nonce_sequence = uint64(nonce)`.
8. Verify nonce, payer ETH balance, and expiry:
...
9. Mempool threshold: gas payer's pending count below node-configured limits.
...
3. Decode packed nonce: `nonce_key = uint192(nonce >> 64)`, `nonce_sequence = uint64(nonce)`. If `nonce_key != NONCE_KEY_MAX`, increment nonce in Nonce Manager storage for `(from, nonce_key)`. If `nonce_key == NONCE_KEY_MAX`, skip (nonce-free mode).
...
The create entry only supports runtime bytecode. [EIP-7702](./eip-7702.md)-style delegation is set via delegation entries (type `0x02`) in `account_changes`.
...
interface IAccountConfig {
    ...
    event DelegationApplied(address indexed account, address indexed target, uint64 sequence);
    ...
    function createAccount(bytes32 userSalt, bytes calldata code, Owner[] calldata initialOwners) external returns (address);
    function getAddress(bytes32 userSalt, bytes calldata code, Owner[] calldata initialOwners) external view returns (address);
