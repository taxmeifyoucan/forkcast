...
`importAccount(address account, Actor[] calldata initialActors, bytes calldata signature)` is a one-time call that registers an already-deployed account into the Account Configuration Contract with an initial actor set. The account must not already be initialized. The `signature` is validated against the account via [ERC-1271](./eip-1271.md) `isValidSignature`, binding the initial actor set to the account's existing authorization logic.
