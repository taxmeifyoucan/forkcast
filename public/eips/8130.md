...
| **Existing Smart Contracts** | Already-deployed accounts (e.g., [ERC-4337](./eip-4337.md) wallets) call `importAccount()` on the Keystore contract themselves; the wallet's code returns the actor set to install via `confirmKeystoreImport()` | Wallet-defined |
...
|-----|-------|------|---------|
| 0 | `0x01` | OPERATOR | Ungated initiation: `sender_auth`; may originate transactions to any `call.to`. Carries **operational** authority, which also governs account-level signing ([ERC-1271](./eip-1271.md)); see [Signature Verification](#signature-verification) |
| 1 | `0x02` | SELF_PAYER | Self-pay gas: authorizes paying the account's own gas when `payer == sender` |
| 2 | `0x04` | SPONSOR_PAYER | Sponsor gas: authorizes acting as `payer_auth` for a different sender (`payer != sender`) |
| 3 | `0x08` | POLICY | Gated initiation: `sender_auth`; may originate transactions only to the actor's `policy_manager` (see [Actor Policies](#actor-policies)). Optional grant |
| 4 | `0x10` | NONCE | Permits a restricted actor to use sequenced `nonce_key`s for sender-context transactions (see [Actor Nonce Scope](#actor-nonce-scope)). Optional grant |
| 5–15 | | (spare) | Reserved for future pure grants |

The core grants occupy bits 0–2; the optional `POLICY` and `NONCE` grants trail them. The canonical bit assignment is `Scopes.sol` in the canonical repository; this table MUST match it.
...
- **What is stored** is decided by the **length** of `policyData` on the authorizing change. `policyData` MUST be either empty (a 32-byte packed record: config only) or exactly `manager` (20 bytes) ‖ `commitment` (32 bytes) (an 84-byte packed record: config + policy), written verbatim to `policy_manager` / `policy_commitment`; any other length is rejected. Neither field need be nonzero: a zero `commitment` is a valid "no parameters" value, and a zero `manager` gates the key to `address(0)`, leaving no productive call target. An authorize with empty `policyData` clears any prior policy slots. The Keystore does not consult scope bits for this.
- **Whether the sender is gated** is decided by the **protocol** from the `POLICY` scope bit (and the absence of `OPERATOR`, see [Actor Scope](#actor-scope)). A `POLICY` actor with no stored `manager` is gated to `address(0)`; an actor with stored policy but no `POLICY` bit is not gated (its manager may still read the commitment, e.g. an external-pull caller).

| `POLICY` set (no `OPERATOR`) | Gate: `call.to` MUST equal | `policyData` at authorize |
|----------------|----------------------------|---------------------------|
| no | (no gate) | empty, or `manager` (20) ‖ `commitment` (32) |
| yes | the stored `manager` (`address(0)` = no productive target) | empty (gates to `address(0)`), or `manager` (20) ‖ `commitment` (32) |

Policy vocabulary lives in the commitment and manager logic.

A policy-bearing actor may call exactly one target: its configured `manager`. The contract at that target reads the actor's `commitment` (via [`getPolicyCommitment`](#ikeystore) or `getActorWithPolicy`), validates presented parameters against it, enforces *what* the call may do, and carries out the approved action. The protocol's only responsibility is the single-target gate.
...
**Scope.** `POLICY` is the gated initiation grant. How it relates to `OPERATOR` is defined in [Actor Scope](#actor-scope). It MAY combine with `SELF_PAYER` so a session key can self-pay gas (bounded by balance and `expiry`). It MAY also combine with `NONCE` so a policy key can use a sequenced `nonce_key`.
...
`NONCE` (`0x10`) grants a restricted actor access to sequenced `nonce_key`s for **sender-context** transactions; it does not apply to `payer_auth`. The protocol enforces this during validation (see [Validation](#validation)):
...
`importAccount()` is a one-time call that registers an already-deployed account into the Keystore contract with an initial actor set. It is **not** signature-relayable and takes no parameters: the account itself MUST be `msg.sender`, and the actor set comes from the account's own code. Authority is the conjunction of (a) the account calling, and (b) the account's code returning the actor set plus the digest of that set from a single `view` callback:
...
Keystore `STATICCALL`s `confirmKeystoreImport()` on `msg.sender`, validates the returned set (non-empty, strictly ascending `actorId`s, `authenticator >= K1_AUTHENTICATOR`, `policyData` empty or 52 bytes), and installs it only when the returned `digest` equals `computeImportDigest(account, initialActors)`. The call is rejected when:

- The account already has 8130 state: **any** change-sequence channel or epoch is non-zero. Import is a one-time bootstrap. A locked account is always caught here — a `lock` is a signed local config change, so it has advanced `local_sequence` and the account is considered initialized (see [Account Lock](#account-lock)) — so there is no separate lock check on import.
- `confirmKeystoreImport()` reverts, cannot be ABI-decoded (a wallet that does not implement it, a constructor, or a plain EOA), or returns a `digest` that is not `computeImportDigest` of the returned set (`ImportNotConfirmed`). A malformed set fails with its specific error (`NoInitialActors` / `ActorsNotSortedOrDuplicate` / `InvalidAuthenticator` / `InvalidPolicyData`) rather than `ImportNotConfirmed`; this ordering is normative.

Import carries **no `chainId`**: it is a live `msg.sender` call plus a live confirm, so the chain is already fixed by where the transaction lands. A wallet that wants to bind import to a specific chain checks `block.chainid` inside `confirmKeystoreImport`.

The digest is a typed ([EIP-712](./eip-712.md)-style) `ActorInitialization` hash that omits the EIP-712 domain separator (anti-phishing) and binds the account's own actorId (`bytes32(uint256(uint160(account)))`, so a returned digest cannot be reused for another account) and each initial actor with `expiry = 0` (imported actors are always non-expiring; an actor-provided expiry MUST NOT be accepted). `policyData` follows `authorizeActor`'s rule and, unlike create, `manager = account` is expressible here. On success `importAccount` sets `DEFAULT_EOA_REVOKED` (parity with `createAccount`); to keep using the native key past import, include the self-actorId as a `K1_AUTHENTICATOR` entry in the returned set. Exact typehashes and digest construction are defined in the canonical repository (`src/Keystore.sol`).

Wallets that do not implement `IKeystoreImport` cannot be imported until they upgrade. Because `importAccount` is reachable through any `execute()`-style path the wallet exposes, a wallet SHOULD gate the return of a valid `(digest, actors)` pair on its own owner-controlled import entry (e.g. a transient-storage handshake set only by an `onlyOwner importToKeystore()`), or accept that anyone who can drive `execute` can finalize the import. After import, the actor set in the Keystore is the source of truth: subsequent owner changes on the wallet do not propagate, and wallets should route their own signature validation through `authenticateActor`.
...
**Calldata floor.** `tx_payload_cost` above is the *standard* [EIP-2028](./eip-2028.md) rate only. [EIP-7623](./eip-7623.md) also imposes a per-transaction **floor** so a transaction that posts many bytes relative to its execution cannot drive the maximum block size back up. Because an 8130 transaction has no single `data` field — its bytes live in `account_changes`, `sender_auth`, `calls`, `metadata`, and `payer_auth` — the floor is evaluated over the serialized transaction. Let `payload_tokens = zero_bytes + 4 * nonzero_bytes` over the bytes `tx_payload_cost` covers (so `tx_payload_cost == 4 * payload_tokens`), and `execution_gas_used` the gas used by `calls`:
...
A transaction whose `gas_limit` is below the floor branch is invalid at [Mempool Acceptance](#mempool-acceptance). The `payer_auth` bytes are floored the same way within `payer_auth_cost` (charged to the payer, outside `gas_limit`), closing the equivalent vector on that field. The floor is normative **structure** under both [adoption profiles](#adoption-profiles), not a repriceable constant: an L2-profile chain MAY change the `10` per-token rate but MUST NOT set it below `4` or drop the floor.
...
This is the same hash-the-leaves-then-hash-the-list scheme used for the signed batch and import digests. Each actor's leaf commits `actorId || authenticator || scope || policyData`: the 32-byte `actorId`, 20-byte `authenticator`, 2-byte `scope`, and the `policyData` bytes (empty, or exactly 52 bytes — `manager (20) ‖ commitment (32)` — independent of `scope`). Fixed-width 32-byte leaves make the commitment unambiguous by construction and linear in the actor count. Expiry does not participate (always `0` at create). The required strictly-ascending `actorId` ordering makes the commitment canonical.

Off-chain address prediction (`computeAddress`) MUST apply the **same** initial-actor validation as create — non-empty set, strictly ascending (non-zero, non-duplicate) `actorId`s, each `authenticator >= K1_AUTHENTICATOR`, and `policyData` empty or exactly 52 bytes — so a predicted address always corresponds to an actor set that create will accept. Otherwise an address could be derived and prefunded for a set a later create would reject, stranding the funds.
...
|-------------|------|-----------|-------------|
| `0` | `AuthorizeActor` | `abi.encode(bytes32 actorId, ActorConfig config, bytes policyData)` (see [`ActorConfig`](#ikeystore)) | Upsert an actor. Writes `actor_config` with `authenticator`, `expiry` (`0` = no expiry; on the unsequenced/JIT path an already-lapsed non-zero expiry causes the grant to be **silently skipped** — not applied, no revert — otherwise the grant installs **inert** — see [Config Change Authorization](#config-change-authorization)), and `scope` verbatim (unknown scope bits stored as-is), reserved bytes zeroed. `policyData` MUST be empty (clears the policy slots) or exactly 52 bytes `manager ‖ commitment` (writes them); attachment is by length, not by any scope bit. Does **not** enforce scope-combination exclusivity (that is use-time / protocol). Emits `ActorAuthorized` whose `actorData` is 32 bytes (`authenticator ‖ expiry ‖ scope ‖ reserved`) when no policy was attached, or 84 bytes (appending `manager ‖ commitment`) when it was. |
...
Session keys often need narrow authority: only this token, only this much per day, only this action. `POLICY` makes gated initiation a first-class grant, distinct from ungated `OPERATOR`: the key may originate transactions, but only to a single call target, bound to a signed opaque **commitment**. The protocol's only policy responsibility is the single-target gate plus storing the commitment. Allowing `POLICY | SELF_PAYER` lets a session key self-pay; note this exposes the account's full ETH balance to gas spend (see [Payer Modes](#payer-modes)).
...
interface IKeystore {
    ...
    // non-expiring; add expiry post-deployment via a config change). policyData: empty, or manager[20] || commitment[32]
    // (by length, independent of scope).
    struct InitialActor {
        bytes32 actorId;
        address authenticator;
        uint16 scope;
    ...
    // installs only when digest == computeImportDigest(msg.sender, actors). No chainId (live call, not a signature).
