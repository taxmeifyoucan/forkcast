...
**Note on `USABLE_BYTES_PER_FIELD_ELEMENT`**: Field elements are interpreted big-endian (see [execution-specs](https://github.com/ethereum/execution-specs/blob/6137bb32f9b8eda6194096992cb97b28be06957d/src/ethereum/crypto/kzg.py#L102)). This EIP sets the first byte (MSB) to zero to ensure that every 32-byte element is a canonical BLS field element (`value < BLS_MODULUS`).
...
- `blockAccessList` (added in EIP-7928)
- `transactions`
...
Execution-payload data includes `blockAccessList` (from EIP-7928) and `transactions`.
