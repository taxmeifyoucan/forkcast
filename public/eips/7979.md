...
Constraints on `MAGIC` code MUST be validated at `CREATE` time, in time and space linear in the size of the code.  To that end a canonical EVM contract implementing an algorithm validating these constraints MUST be placed on the blockchain and the `MAGIC` header MUST include the address of this contract.  _(Note that this mechanism could easily be extended to support user-defined validation contracts.)_
...
* *Immediate arguments* for jump destinations would improve performance but increase complexity.  See [EIP-8013: Static relative jumps and calls for the EVM](./eip-8013) for a complementary proposal using immediate arguments.  

* *Code sections* or other structural constraints would impose syntactic restrictions that inhibit optimization.  See [EIP-3540: EOF - EVM Object Format](./eip-3540) and [EIP-4750: EOF - Functions](./eip-4750) for complementary proposals that provide more structure.  
...
By marking _MAGIC_ contracts as `valid` we are promising that their control-flow is static, and the many tools that traverse the control flow will not see their efficiency impracticaly degraded by dynamic jumps, including AOT and JIT compilers for clients themselves.  The intention is to mitigate the chicken-and-egg cycle of the tools working around the EVM's dynamic control flow to recover subroutines, while the compilers work around it to implement subroutines, with neither group much aware of the other.  All of which makes for a cycle of needless complexity and debt.  With this proposal it will at least and at last become possible to write static EVM code and break the cycle.  
...
Therefore this proposal _does not_ require a client to maintain two interpreters -- validation is done before the interpreter runs, and the interpreter is not required to care whether the code is `valid`.  Neither does this proposal require a client to implement validation code — it will already be on the blockchain.  
