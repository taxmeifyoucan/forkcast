...
The EVM is a stack machine, but most real machines are register machines.  Both routes — an interpreted register code, and ahead-of-time compilation to machine code — are measured in this proposal's assets (`riscv/`): executed RISC-V instructions, counted exactly, on two kernels that bracket the workload space — *mul chain*, a loop of dependent multiplies, nearly pure arithmetic; *call tree*, a recursive tree of calls that computes almost nothing.  Lower is better, and the parentheses give each cell's gain over the status quo: interpreted bytecode at today's 256-bit word width.  The 64-bit columns use proposed 64-bit arithmetic opcodes.  Read *down* for what static control flow buys — each row is a cheaper execution tier the proofs permit.  Read *across* for what the 64-bit opcodes buy.  The corner cells are both together.

|                      | mul chain, 256 | mul chain, 64 | call tree, 256 | call tree, 64 |
|----------------------|---------------:|--------------:|---------------:|--------------:|
| interpreted bytecode | 6,122,597 | 4,492,253 (1.4x) | 7,833,101 | 7,360,257 (1.1x) |
| register IR          | 4,081,563 (1.5x) | 1,856,126 (3.3x) | 3,669,063 (2.1x) | 2,450,626 (3.2x) |
| compiled (AOT)       | 1,315,924 (4.7x) | 120,641 (50.8x) | 1,119,924 (7.0x) | 405,641 (19.3x) |
...
To confirm that the instruction counts above are what a prover actually pays, we ran the same programs in Zisk, Polygon's RISC-V zkVM.  Every program cost the prover exactly its instruction count from the table, plus 444 steps of fixed startup.  So the ratios in the table are ratios of proving costs: the same contract is 5x to 50x cheaper to prove compiled than interpreted.  Details and reproduction under `riscv/` in the assets.
...
The machine state gains one field, and JUMPDEST analysis one set:
