...
`leafCount` MAY be zero. An anchor state with no accounts converts to exactly that: a snapshot whose `pbtRoot` is the empty-tree hash of [EIP-8297](./eip-8297.md), 32 zero bytes, with no leaf records, and an empty preimage file. Such a snapshot is not a special case for the dual-check: it passes only against an anchor whose `stateRoot` is the empty MPT root, and consensus anchoring rejects it against any other.
