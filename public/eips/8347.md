...
- **Code.** A code change MUST recompute the chunk leaves for the new code and MUST delete every header chunk leaf of the previous code that the new code does not cover. Replay never deletes an overflow chunk, and never needs to. The one path that removes deployed code is `SELFDESTRUCT`, which [EIP-6780](./eip-6780.md) confines to the account's creation transaction, and [EIP-7928](./eip-7928.md) records an account destroyed within a transaction without nonce or code changes, so replay never inserted that code's chunks in the first place. Every other code change replay sees either creates code, which only adds leaves, or writes an [EIP-7702](./eip-7702.md) delegation indicator, which is 23 bytes and so occupies chunk 0 alone. Clearing a delegation is therefore confined to the header stem, deleting the chunk leaf at sub-index `CODE_OFFSET` while updating the `code_hash` and basic-data leaves to the values of a codeless account.
- **Accounts.** If, after a block's writes are applied, an account's basic data would hold `nonce == 0`, `balance == 0`, and `code_size == 0`, replay MUST delete the account: its header stem together with any leaves under the shared prefix `STORAGE_ZONE || key_hash(address)`. As above, replay holds no overflow chunk for such an account, so the `CODE_ZONE` limb of [EIP-8297](./eip-8297.md)'s account deletion has nothing to remove. The trigger is exact in both directions, since [EIP-7523](./eip-7523.md) leaves no empty account in the MPT and a non-empty account is never absent from it.
