...
The proposed Binary Tree stores key-value entries where the key is bytes (arbitrary length, but this EIP guarantees that it will be prefix-free) and the value is bytes32. The first byte of the tree denotes storage type, the next bytes of the key define the entry stem, and the last byte is the subindex in that stem. If two keys have the same stem, they live in the same "big branch" — this co-locates groups of 256 keys (i.e: keys with the same first 31-bytes).
